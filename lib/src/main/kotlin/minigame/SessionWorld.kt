...
class SessionWorld() {
    ...
    private fun load(): World {
        ...
        val mapPath: File = File(Bukkit.getWorldContainer(), mapName)

        try {
            val zis: ZipInputStream = ZipInputStream(FileInputStream(zipPath))
            val entry: ZipEntry? = zis.nextEntry

            while (entry != null) {
                val file: File = File(mapPath, mapName)
        ...
        val worldCreator: WorldCreator = WorldCreator(mapName)
    ...
    fun unload() {
        ...
        val mapPath: File = File(Bukkit.getWorldContainer(), world.name)
